...
void target_x_y_array_callback(const std_msgs::Float32MultiArray::ConstPtr& target_x_y_array){
  ...
  buf_target_x_y_array[2*i+1] = target_x_y_array->data.at(2*i+1);
  }
}
...
void view_point_x_y_theta_array_callback(const beginner_tutorial::carPosition::ConstPtr& veiw_point_x_y_theta_array){
  ...
  ROS_INFO("buf_target_x_y_array : %lf",buf_target_x_y_array[2*i+1]);
  ...
  if (temp_horizontal_diviation < abs(horizontal_diviation.data)){
    f = line_through_pos_and_view_poin(buf_target_x_y_array[2*i], buf_target_x_y_array[2*i+1], buf_x_y_theta_array[0], buf_x_y_theta_array[1], delta_x, delta_y);
  ...
  ROS_INFO("horizontal_diviation: [%.1f] delta_y : [%lf]", horizontal_diviation.data,delta_y);
